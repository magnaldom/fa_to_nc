...
for d in range(0,num_jour):
	...
	for t in range(t_deb, t_fin):
		fichier.write("/home/magnaldom/vortex/arome/3dvarfr/%s/%s%s%sT0000P/forecast/historic.arome.franmg-01km30+"  %(expe,date.year, str(date.month).zfill(2),str(date.day).zfill(2)) + str(t).zfill(4)  + ":00.fa" + "\n")
